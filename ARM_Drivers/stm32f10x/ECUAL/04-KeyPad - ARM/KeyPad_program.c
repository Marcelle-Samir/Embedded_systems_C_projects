...
u8 KEYPAD_GetPressedKey(void)
{
	...
	for(RowIdx=0;RowIdx<KEYPAD_ROWS_NUMBER;RowIdx++)
	{
		...
		DIO_SetPinVal(Keypad_RowsPort[RowIdx],Keypad_RowsPin[RowIdx],0);
		
		/*Check columns if pressed*/
		for(ColIdx=0;ColIdx<KEYPAD_COLUMNS_NUMBER;ColIdx++)
		{
			PinState=DIO_GetPinVal(Keypad_ColumnsPort[ColIdx],Keypad_ColumnsPin[ColIdx]);
		...
		/*Deactivate the current row*/
			DIO_SetPinVal(Keypad_RowsPort[RowIdx],Keypad_RowsPin[RowIdx],1);
